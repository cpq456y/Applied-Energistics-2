...
public class DualityInterface implements IGridTickable, IStorageMonitorable, IInventoryDestination, IAEAppEngInventory, IConfigManagerHost, ICraftingProvider, IUpgradeableHost, IAEFluidInventory {
    ...
    public void readFromNBT(final NBTTagCompound data) {
        ...
        switch (unlockEventType) {
            ...
            case 1: this.unlockEvent = UnlockCraftingEvent.PULSE; break;
                ...
                try {
                    ...
                    if (sm != null && Platform.canAccess(proxyable.getProxy(), this.mySource)) {
                        if (this.isBlocking() && !sm.getInventory(AEApi.instance().storage().getStorageChannel(IItemStorageChannel.class)).getStorageList().isEmpty()) {
                            ...
                            boolean allItemsCanBeInserted = true;
    ...
    public boolean isBusy() {
        ...
        if (this.isBlocking()) {
            ...
            for (final EnumFacing s : possibleDirections) {
                ...
                final InventoryAdaptor ad = InventoryAdaptor.getAdaptor(te, s.getOpposite());
                if (ad != null) {
                    if (Platform.isModLoaded("actuallyadditions") && Platform.GTLoaded && te instanceof IPhantomTile) {
    ...
    private boolean getRedstoneState() {
        if (redstoneState == YesNo.UNDECIDED) {
            TileEntity entity = this.iHost.getTileEntity();
