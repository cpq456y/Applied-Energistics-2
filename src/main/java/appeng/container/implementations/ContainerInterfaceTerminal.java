...
public class ContainerInterfaceTerminal extends AEBaseContainer {
    ...
    public ItemStack transferStackInSlot(EntityPlayer p, int idx) {
        if (Platform.isClient()) {
            Slot playerSlot = this.inventorySlots.get(idx);
            if (playerSlot instanceof AppEngSlot && ((AppEngSlot) playerSlot).isPlayerSide()) {
                for (Object slotObj : this.inventorySlots) {
                    if (slotObj instanceof SlotDisconnected && !((Slot) slotObj).getHasStack()) {
                        SlotDisconnected slotDisconnected = (SlotDisconnected) slotObj;
                        // Signal the server to move the pattern.
                        Slot playerAppEngSlot = (AppEngSlot) playerSlot;
                        PacketInventoryAction packet = new PacketInventoryAction(InventoryAction.PLACE_SINGLE,
